...
import { orderByNameZA } from '../../../redux/actions/actionPokemon'
...
const ContainerZA = () => {
    ...
    const pokemons = useSelector(state=>state.pokemonReducer.orderByNameZA)
    const nextPage = () => {
        setOffset(offSet + 12);
        setLimit(limit + 12)
    };
    ...
    useEffect(()=>{
        dispatch(orderByNameZA(offSet,count,limit))
